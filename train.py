...
if __name__ == '__main__':
    ...
    if evaluate_validation:
        ...
        test_dataset = CircuitPairDataset(np_data[train_len:train_len + valid_len],
                                          np_target[train_len:train_len + valid_len], transform=transform,
                                          reflect=False)
        valid_dataset = CircuitPairDataset(np_data[train_len + valid_len:], np_target[train_len + valid_len:],
                                           transform=transform, reflect=False)
    ...
    for epoch in range(epochs):
        ...
        if evaluate_validation:
            ...
            if ((epoch + 1) % 10 == 0) or ((epoch + 1) == epochs):
                torch.save({'epoch': epoch + 1, 'state_dict': model.state_dict(), 'loss_history': losses},
