...
def u3(t, p, l):
    return np.array([[np.cos(t / 2), -np.exp(1j * l) * np.sin(t / 2)],
                     [np.exp(1j * p) * np.sin(t / 2), np.exp(1j * (l + p)) * np.cos(t / 2)]])
...
print(
    'True dynamical decoupling (DD) sequences, such as XYXY achieve first-order suppression of decoherence (ideal is 0):',
    process_seq(xyxy_test)[0])
...
print(
    'All 95% confidence intervals overlap, and none of the values are near zero, showing first-order dynamical decoupling is not achieved by the deep learning sequences.')
