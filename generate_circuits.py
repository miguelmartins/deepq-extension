...
def identity_circuit(num_gates: int) -> List[str]:
    ...
    for _ in range(num_gates - 1):
        theta, phi, lamb = np.random.choice(parameter_space), \
                           np.random.choice(parameter_space), \
                           np.random.choice(parameter_space)
        gates.append(UGate(theta, phi, lamb))
        identity.append(identity_string(theta, phi, lamb))

    theta, phi, lamb = invert_circuit_ubasis(gates)[0][0]._params
    identity.append(identity_string(theta, phi, lamb))
    return identity
    ...
    for i in range(n):
        ...
        for z in zeros:
            if z[1] != circuit_arr.shape[1] and z[1] - z[0] > 1:
                identity = identity_circuit(z[1] - z[0])
...
def make_family(ind, backend, length=16, n=5, directory='./supremacy_all_5_unique/'):
    try_again = True
    while try_again:
        try_again = False
        family = []
...
if __name__ == '__main__':
    ...
    num_circuits = 1000
    thread_args = np.array([(i, backend) for i in np.arange(num_circuits)])
    with Pool(28) as p:
        p.starmap(make_family, thread_args)
